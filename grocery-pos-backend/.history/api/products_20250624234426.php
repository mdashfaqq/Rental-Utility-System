...
function getProducts($db)
{
    ...
    while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
        $products[] = array(
            ...
            "unit" => $row['unit'] ?? 'piece',
            "smallUnit" => $row['small_unit'] ?? '',
            "conversionFactor" => $row['conversion_factor'] ? floatval($row['conversion_factor']) : null,
            "minQuantity" => floatval($row['min_quantity'] ?? 1)
        );
    }
...
function createProduct($db)
{
    ...
    if (!empty($data->name) && (isset($data->unit_price) || isset($data->selling_price))) {
        try {
            ...
            // Fix: Don't use fallback logic that makes both prices the same
            ...
            $min_quantity = isset($data->minQuantity) ? floatval($data->minQuantity) : 1;
        ...
        echo json_encode(array("message" => "Incomplete data. Name and at least one price are required."));
    }
}
    ...
    if (!empty($data->id)) {
        try {
            $query = "UPDATE products SET name = :name, barcode = :barcode, description = :description, 
                      ...
                      small_unit = :small_unit, conversion_factor = :conversion_factor, min_quantity = :min_quantity
                      WHERE id = :id";
