...
function createSubCategory($db) {
    ...
    if (!empty($data->name) && !empty($data->category_id)) {
        ...
        if($stmt->execute()) {
            ...
            echo json_encode(array("message" => "Unable to create sub-category", "error" => $error_info));
        }
    } else {
        http_response_code(400);
        $debug_info = array(
            "name" => isset($data->name) ? $data->name : "missing",
            "category_id" => isset($data->category_id) ? $data->category_id : "missing",
            "data_received" => $data
        );
        error_log("Validation failed: " . print_r($debug_info, true));
        echo json_encode(array("message" => "Sub-category name and category ID required", "debug" => $debug_info));
    }
}
