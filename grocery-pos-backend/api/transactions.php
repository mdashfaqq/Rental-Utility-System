...
function getTransactions($db) {
    ...
    while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
        ...
        // Create base transaction structure if not exists
        ...
        // Add items only if product_id is present
...
function createTransaction($db) {
    ...
    if (!empty($data->items) && !empty($data->total)) {
        try {
            ...
            $query = "INSERT INTO transactions (transaction_number, bill_number, subtotal, tax_amount, discount_amount, total_amount, payment_method, user_id, customer_name) 
                      VALUES (:transaction_number, :bill_number, :subtotal, :tax_amount, :discount_amount, :total_amount, :payment_method, :user_id, :customer_name)";
            ...
            $stmt->bindParam(":customer_name", $data->cashier_name);
            ...
            foreach ($data->items as $item) {
                $productId = $item->id;
                    ...
                    // 🔁 Handle custom product if ID starts with 'custom-'
                    if (!is_numeric($item->id) && strpos($item->id, 'custom-') === 0) {
                        error_log("🛠 Detected custom product: " . json_encode($item));

                        // Validate that item_code is present
                        if (!isset($item->item_code) || empty($item->item_code)) {
                            throw new Exception("Custom product is missing item_code");
                        }

                        // Insert custom product including item_code
                        $customInsert = "INSERT INTO custom_products (name, price, unit, item_code) 
                                        VALUES (:name, :price, :unit, :item_code)";
                        $customStmt = $db->prepare($customInsert);
                        $customStmt->bindParam(":name", $item->name);
                        $customStmt->bindParam(":price", $item->price);
                        $customStmt->bindParam(":unit", $item->unit);
                        $customStmt->bindParam(":item_code", $item->item_code);
                        $customStmt->execute();

                        $productId = $db->lastInsertId(); // Use this new ID as product_id

                        error_log("✅ Custom product inserted with ID: $productId and code: {$item->item_code}");
                    }
                ...
                if (is_numeric($item->id)) {
                    ...
                    $movementStmt->bindParam(":product_id", $productId);
