...
function login($db) {
    ...
    if (!empty($data->username) && !empty($data->password)) {
        ...
        if ($stmt->rowCount() > 0) {
            ...
            // Prefer hashed passwords; fallback to plaintext match if the DB entry was saved without hashing
            $isValid = password_verify($data->password, $user['password']) || $data->password === $user['password'];
    ...
    if (!empty($data->username) && !empty($data->email) && !empty($data->password)) {
        ...
        if ($check_stmt->rowCount() > 0) {
            ...
            // echo json_encode(array("message" => "User already exists"));
             echo json_encode(array("message" => "User already exists with this username or email"));
            return;
        }
        
        // $query = "INSERT INTO users (username, email, password, role) VALUES (:username, :email, :password, :role)";
        $query = "INSERT INTO users (username, email, password, role, is_active, created_at) VALUES (:username, :email, :password, :role, 1, NOW())";
        $stmt = $db->prepare($query);
